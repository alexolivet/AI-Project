from flask import Flask, render_template, jsonify, request, got_request_exception, session, send_from_directory, send_file, url_for
import config
...
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder # crafts prompts for our llm
...
#                                 ))
  Chromaclient_specific=chromadb.PersistentClient(path=f"./vectorDatabase/{specific_directory}")
  return Chromaclient_specific

def view_chroma_database_specific():
  global collections_specific
  collections_specific = Chromaclient_specific.list_collections()  # get the list of collections
...
def get_news(query):
    ...
    key='fdd256696ded40c7864116f053b97ee3'
    ...
    try:
     ...
     for i in response_json['articles']:
        fetchedNews=f"Title: {i['title']} ,Description:  {i['description']},URL: {i['url']}"
...
tools = [
    {
        "name": "get_news",
        "description": "Use this function to get the latest news from NewsApi",
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
...
def setup_stabilityai():
    getAccount = requests.get("https://api.stability.ai/v1/user/account", headers={
        "Authorization": f"Bearer {api_key_stability}"
        })
    getBalance = requests.get("https://api.stability.ai/v1/user/balance", headers={
        "Authorization": f"Bearer {api_key_stability}"
        })
...
def setup_stabilityaiv2():
    getAccount = requests.get("https://api.stability.ai/v1/user/account",verify=False, headers={
        "Authorization": f"Bearer {api_key_stability}"
        })
    getBalance = requests.get("https://api.stability.ai/v1/user/balance",verify=False, headers={
        "Authorization": f"Bearer {api_key_stability}"
        })
    getSystemStatus = requests.get("https://stabilityai.instatus.com/summary.json",verify=False, headers={
        "Authorization": f"Bearer {api_key_stability}"
        })
...
def ollama_chat_history():
    ...
    return render_template('ollama_chat_history.html', page="Ollama Chatbot History",**locals())
...
def ollama_function_call():
    ...
    if request.method == 'POST':
        ...
        model_fc = OllamaFunctions(model="dolphin-phi",
        ...
        return jsonify(content=contentToAjax), 200
    return render_template('ollama_function_call.html', page="Ollama Function Call",**locals())
            ...
            if len(user_input_file)>=1:
             for file in user_input_file:
                if allowed_file(file) is True:
                    ...
                    loader = PyPDFLoader(f"/home/alexaj14/Documents/AI-Project/static/pdf/{file}")
...
def chroma_ingest():
    if request.method == 'POST':
        try:
            ...
            semantic="No"
            store_chroma_in_sql_collection(collection_name_from_form,str(uuid),metadata,int(chunk_size),int(chunk_overlap),embed_model,str(collection_name_from_form),str(urls),str(user_input_file),semantic)
...
def chroma_view_database():
    ...
    if request.method == 'POST':
      if 'selected_collection' in request.form:
        try:
            collection_name_from_form = request.form.get("collection_name_from_form")
            global Chromaclient_specific
            get_chroma_client_specific(collection_name_from_form)
            selected_collection = Chromaclient_specific.get_collection(name=collection_name_from_form)
    ...
    return render_template('chroma_view_database.html', page="chroma_view_database", **locals())
...
def load_embedding_ollama():
    ...
    return render_template('load_embedding_ollama.html', page="Load-Embedding",**locals())
...
def sync_local_with_chroma_db(i):
    try:
     ...
     if len(source_file_names)<1:
        ...
        print(f"Collection'{collection.name}' without source file has been deleted.")
        content=f"Collection'{collection.name}' without source file has been deleted."
     else:
         for source_file_name in source_file_names:
          id_to_delete = [id for id, metadata in zip(ids, metadatas) if metadata.get('source') == source_file_name]
...
def imagevariation():
    ...
    if request.method == 'POST':
         ...
         try:
            ...
            response = client.images.create_variation(model="dall-e-2",image=byte_array,n=1,size="512x512")
...
def vision_ollama():
    ...
    return render_template('vision_ollama.html', page="LLaVA vision encoder",**locals())
...
def imgvar_stabilityai():
    ...
    return render_template('imgvar_stabilityai.html', page="Image Variation",**locals())
...
def imgvar_stabilityai_v2():
    ...
    return render_template('imgvar_stabilityai_v2.html', page="Image Variation",**locals())
...
def imageToVideo_stabilityai():
        ...
        if "prompt" in request.form:
            ...
            try:
                ...
                if response.status_code == 200:
                 ...
                 save_generate_id_stabilityai(gen_id_from_request,int(seed),float(cfg_scale),int(motion_bucket_id),user_input)
                 RowIdForSelect= get_generation_id_row(gen_id_from_request)
                 print(RowIdForSelect)
                 return jsonify(content=content,gen_id_from_request=gen_id_from_request,RowIdForSelect=RowIdForSelect), 200
            except Exception as e:
                print(e)
                content = f"Oops something went wrong.: {response.text}"
                # raise Exception("Non-200 response: " + str(response.text))
        ...
        if "generation_id" in request.args:
            ...
            try:
                ...
                #     print("note a video file")
                    ...
                    with open(f'./static/video/videoStabilityAI_{num}.mp4', 'wb') as file:
    ...
    return render_template('imageToVideo_stabilityai.html', page="Image to Video",**locals())
...
def chatbotimage():
   if request.method == 'POST':
        ...
        try:
            ...
            response = client.images.generate(model="dall-e-3",prompt=user_input,n=1,quality="standard",size="1024x1024")
            # content = response.choices[0].message["content"]
            # Generate images using DALL-E
            content = response.data[0].url
            # count_tokens = response.usage.total_tokens
...
def save_generate_id_stabilityai(token_val,seed,cfg_scale,motion_bucket_id,image_path):
    try:
       create_dt=datetime.datetime.now()
       conn = get_db_connection() 
       cur = conn.cursor()  
       cur.execute("INSERT into gen_id (token_val,create_dt,seed,cfg_scale,motion_bucket_id,image_path) values (?,?,?,?,?,?)",(token_val,create_dt,seed,cfg_scale,motion_bucket_id,image_path))  
       conn.commit()  
    except sqlite3.Error as er:
        print('SQLite error: %s' % (' '.join(er.args)))
        print("Exception class is: ", er.__class__)
...
def update_generate_id_stabilityai(id):
    try:
       ...
       conn = get_db_connection() 
       cur = conn.cursor()  
       cur.execute('UPDATE gen_id SET is_retreived = ?, update_dt = ? WHERE id = ?', ['1', update_dt,id])  
       conn.commit()  
    except sqlite3.Error as er:
        print('SQLite error: %s' % (' '.join(er.args)))
        print("Exception class is: ", er.__class__)
...
def store_chroma_in_sql_collection(collection_name,uuid,metadata,chunk_size,chunk_overlap,embed_model,persist_directory,url,pdf,semantic):
    try:
       create_dt=datetime.datetime.now()
       conn = get_db_connection() 
       cur = conn.cursor()  
       cur.execute("INSERT into collections (collection_name,uuid,metadata,create_dt,chunk_size,chunk_overlap,embed_model,persist_directory,url,pdf,semantic) values (?,?,?,?,?,?,?,?,?,?,?)",(collection_name,uuid,metadata,create_dt,chunk_size,chunk_overlap,embed_model,persist_directory,url,pdf,semantic))  
       conn.commit()  
    except sqlite3.Error as er:
        print('SQLite error: %s' % (' '.join(er.args)))
        print("Exception class is: ", er.__class__)
